...
def test_returns_zero_if_no_unfinalized_requests(subject, past_blockstamp, chain_config, frame_config):
    subject._has_unfinalized_requests = Mock(return_value=False)
    subject._get_available_eth = Mock(return_value=0)

    result = subject.get_next_last_finalizable_id(True, 100, 0, 0, past_blockstamp, chain_config, frame_config)
...
def test_returns_last_finalizable_id(subject, past_blockstamp, chain_config):
    ...
    assert subject.get_next_last_finalizable_id(True, 100, 0, 0, past_blockstamp, chain_config, frame_config) == 1