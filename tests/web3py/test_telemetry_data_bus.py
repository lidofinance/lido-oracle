...
class TestTelemetryDataBus:
    ...
    def test_send_telemetry__configured__sends_transaction(
        ...
        mock_data_bus_w3.eth.get_balance.assert_called_once_with(account.address)

    @patch('src.web3py.extensions.telemetry_data_bus.sign_and_send_transaction')
    @patch('src.web3py.extensions.telemetry_data_bus.build_transaction_params')
    def test__send_with_retry__mined_on_first_attempt__returns_tx_hash(
        self, mock_build_params, mock_sign_and_send, web3
    ):
        ...
        result = module._send_with_retry(tx, w3_mock, account, timeout=120)
    ...
    @patch('src.web3py.extensions.telemetry_data_bus.sign_and_send_transaction')
    @patch('src.web3py.extensions.telemetry_data_bus.build_transaction_params')
    def test__send_with_retry__pending_then_mined__retries_until_included(
        self, mock_build_params, mock_sign_and_send, mock_monotonic, mock_sleep, web3
    ):
        # start_time=0; after 1st pending check, monotonic()=1 is still within the 120s deadline -> retry.
        mock_monotonic.side_effect = [0, 1]
        w3_mock, account, tx = self._mock_send_retry_env()
        mock_build_params.side_effect = [{'nonce': 1}, {'nonce': 2}]
        first_hash, second_hash = b'\xbb' * 32, b'\xcc' * 32
        mock_sign_and_send.side_effect = [first_hash, second_hash]
        w3_mock.eth.get_transaction.side_effect = [{'blockNumber': None}, {'blockNumber': 7}]
    ...
    @patch('src.web3py.extensions.telemetry_data_bus.sign_and_send_transaction')
    @patch('src.web3py.extensions.telemetry_data_bus.build_transaction_params')
    def test__send_with_retry__timeout_exceeded_while_pending__gives_up_without_raising(
        self, mock_build_params, mock_sign_and_send, mock_monotonic, mock_sleep, web3
    ):
        # start_time=0; check: monotonic()=5 is past the 1s deadline -> give up, return the pending tx hash.
        ...
        module = self._create_module(web3)
        result = module._send_with_retry(tx, w3_mock, account, timeout=1)
    ...
    @patch('src.web3py.extensions.telemetry_data_bus.sign_and_send_transaction')
    @patch('src.web3py.extensions.telemetry_data_bus.build_transaction_params')
    def test__send_with_retry__build_params_fails_then_succeeds__sleeps_then_retries(
        self, mock_build_params, mock_sign_and_send, mock_sleep, web3, caplog
    ):
        w3_mock, account, tx = self._mock_send_retry_env()
        ...
        mock_sign_and_send.assert_called_once()
        mock_sleep.assert_called_once_with(10)
        assert 'DataBus telemetry send failed.' in caplog.text

    @patch('src.web3py.extensions.telemetry_data_bus.time.sleep')
    @patch('src.web3py.extensions.telemetry_data_bus.sign_and_send_transaction')
    @patch('src.web3py.extensions.telemetry_data_bus.build_transaction_params')
    def test__send_with_retry__sign_and_send_fails_then_succeeds__sleeps_then_retries(
        self, mock_build_params, mock_sign_and_send, mock_sleep, web3, caplog
    ):
        w3_mock, account, tx = self._mock_send_retry_env()
        mock_build_params.return_value = {'nonce': 1}
        ...
        w3_mock.eth.get_transaction.return_value = {'blockNumber': 9}
    ...
    @patch('src.web3py.extensions.telemetry_data_bus.sign_and_send_transaction')
    @patch('src.web3py.extensions.telemetry_data_bus.build_transaction_params')
    def test__send_with_retry__get_transaction_fails_then_mined__sleeps_then_retries(
        self, mock_build_params, mock_sign_and_send, mock_sleep, web3, caplog
    ):
        w3_mock, account, tx = self._mock_send_retry_env()
        mock_build_params.return_value = {'nonce': 1}
        tx_hash = b'\x12' * 32
        mock_sign_and_send.return_value = tx_hash
        w3_mock.eth.get_transaction.side_effect = [ValueError('not found'), {'blockNumber': 100}]
    ...
    @patch.object(TelemetryDataBus, '_send_with_retry')
    @patch.object(TelemetryDataBus, '_validate')
    @patch.object(TelemetryDataBus, '_create_web3')
    def test_send_telemetry__timeout_provided__forwarded_to_send_with_retry(
        self, mock_create_web3, mock_validate, mock_send_with_retry, web3, monkeypatch
    ):
        ...
        mock_contract = Mock()
        mock_data_bus_w3.eth.contract.return_value = mock_contract
        mock_send_with_retry.return_value = b'\xab' * 32

        module = self._create_module(web3, data_bus_rpc=DUMMY_RPC, data_bus_address=DUMMY_ADDRESS)
        module.send_telemetry(TelemetryEventId.ORACLE_STARTUP, timeout=42)

        mock_send_with_retry.assert_called_once_with(
            mock_contract.send_message.return_value, mock_data_bus_w3, variables.TELEMETRY_ACCOUNT, 42
        )
