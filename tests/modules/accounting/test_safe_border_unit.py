...
@pytest.fixture()
def subject(web3, chain_config, frame_config, contracts, keys_api_client, consensus_client):
    safe_border = SafeBorder(web3)
    safe_border.chain_config = chain_config
    safe_border.frame_config = frame_config
...
def test_get_associated_slashings_border_epoch(subject, past_blockstamp):
    ...
    assert subject._get_associated_slashings_border_epoch(past_blockstamp) == subject.round_epoch_by_frame(test_epoch) - NEW_REQUESTS_BORDER
...
def test_get_earliest_slashed_epoch_among_incomplete_slashings_all_withdrawable(subject, past_blockstamp):
    validators = [
        create_validator_stub(past_blockstamp.ref_slot - 100, past_blockstamp.ref_slot - 1, True),
        create_validator_stub(past_blockstamp.ref_slot - 100, past_blockstamp.ref_slot - 2, True),
    ]
    subject._get_lido_validators = MagicMock(return_value=validators)
...
def test_get_earliest_slashed_epoch_among_incomplete_slashings_at_least_one_unpredictable_epoch(subject, past_blockstamp):
    non_withdrawable_slot = past_blockstamp.ref_slot + 10
    validators = [
        create_validator_stub(non_withdrawable_slot - 100, non_withdrawable_slot + MIN_VALIDATOR_WITHDRAWABILITY_DELAY, True),
        create_validator_stub(non_withdrawable_slot - 100, non_withdrawable_slot + 1, True),
    ]
    subject._get_lido_validators = MagicMock(return_value=validators)
    subject._find_earliest_slashed_epoch = MagicMock(return_value=1331)
...
def test_get_bunker_start_or_last_successful_report_epoch_no_bunker_start(subject, past_blockstamp):
    ...
    assert subject._get_bunker_start_or_last_successful_report_epoch(past_blockstamp) == past_blockstamp.ref_slot // 32
...
def test_get_last_finalized_withdrawal_request_slot(subject, past_blockstamp):
    ...
    assert subject._get_last_finalized_withdrawal_request_slot(past_blockstamp) == first_slot

def test_get_last_finalized_withdrawal_request_slot_no_requests(subject, past_blockstamp):
    subject._get_last_finalized_request_id = MagicMock(return_value=0)
    
    assert subject._get_last_finalized_withdrawal_request_slot(past_blockstamp) == 0

def test_check_slot_diff(subject):
    slot_a = 1
    slot_b = 319
    slot_c = 320

    assert subject._check_slot_diff(slot_a, slot_b)
    assert not subject._check_slot_diff(slot_a, slot_c)
    assert not subject._check_slot_diff(slot_b, slot_c)
