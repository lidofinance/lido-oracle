...
def test_bunker_mode_associated_slashing_predicted(
    ...
    exit_epoch = past_blockstamp.ref_epoch - int(spec.MIN_VALIDATOR_WITHDRAWABILITY_DELAY)
    ...
    assert subject.get_safe_border_epoch(is_bunker_mode) == (
        subject.round_epoch_by_frame(withdrawable_epoch - int(spec.EPOCHS_PER_SLASHINGS_VECTOR)) - subject.finalization_default_shift
    )
...
def test_bunker_mode_associated_slashing_unpredicted(
    ...
    is_bunker_mode = True
    withdrawable_epoch = past_blockstamp.ref_epoch + 128
    exit_epoch = withdrawable_epoch - int(spec.MIN_VALIDATOR_WITHDRAWABILITY_DELAY)
    activation_epoch = withdrawable_epoch - int(spec.EPOCHS_PER_SLASHINGS_VECTOR) - 2
    ...
    subject._get_last_finalized_withdrawal_request_slot = MagicMock(
        return_value=withdrawable_epoch - int(spec.EPOCHS_PER_SLASHINGS_VECTOR) - 2
    )
    subject.w3.lido_validators.get_lido_validators = MagicMock(
        return_value=[
