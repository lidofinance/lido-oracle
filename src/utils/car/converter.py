...
class CARConverter:
    ...
    def _encode_header(self, roots: list[CID]) -> bytes:
        try:
            header_data = {"version": CAR_VERSION, "roots": roots}
            header_bytes = dag_cbor.encode(cast(dag_cbor.IPLDKind, header_data))
    ...
    def _encode_block(self, cid: CID, block_bytes: BytesLike, block_index: int) -> bytes:
        ...
        if not isinstance(block_bytes, BYTES_LIKE_TYPES):
            raise CAREncodingError(f"Block {block_index}: block bytes must be bytes-like, got {type(block_bytes)}")
    ...
    def create_car_from_data(self, data_bytes: bytes) -> CarFile:
        ...
        return CarFile(car_bytes=car_bytes, root_cid=root_cid.encode(), shard_cid=shard_cid, size=size)
