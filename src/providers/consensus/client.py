...
from src.types import BlockRoot, BlockStamp, Fork, Gwei, SlotNumber, EpochNumber, StateRoot
...
class ConsensusClient(HTTPProvider):
    ...
    @lru_cache(maxsize=1)
    def get_fork(self, state_id: LiteralState | SlotNumber | StateRoot) -> Fork:
        # Making a raw request to the spec endpoint to get all the fields.
        spec, _ = self._get(self.API_GET_SPEC)
        if not isinstance(spec, dict):
            raise ValueError("Expected mapping response from getSpec")
        spec = cast(dict[str, str], spec)

        versions = {}
        for k, v in spec.items():
            if k.endswith("FORK_VERSION"):
                versions[v] =k.split("_")[0]

        data, _ = self._get(
            self.API_GET_FORK,
            path_params=(state_id,),
            # TODO: Check if we need force_raise
        )
        if not isinstance(data, dict):
        ...
        if current_version not in versions:
            raise ValueError(f"Got unexpected {current_version=}")

        return Fork(versions[current_version])  # type: ignore[arg-type]
    ...
    def get_state_view(self, state_id: LiteralState| SlotNumber | StateRoot) -> BeaconStateView:
        ...
        try:
            ...
            view['slot'] = data['slot']
            view['exit_balance_to_consume'] = Gwei(data['exit_balance_to_consume'])
