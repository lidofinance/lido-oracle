...
| `MEMBER_PRIV_KEY_2`                                      | Second candidate private key for soft key rotation (plain EOA member, or a candidate delegate of `DELEGATION_CONTRACT_ADDRESS`). See [delegation guide](docs/delegation.md) | False               | `0x1...`                                     |
| `MEMBER_PRIV_KEY_2_FILE`                                 | A path to the file contained the private key of the second candidate key. It takes precedence over `MEMBER_PRIV_KEY_2`                                                      | False               | `/app/private_key_2`                         |
| `TELEMETRY_PRIV_KEY`                                     | Private key of a dedicated account for sending telemetry to the DataBus. Falls back to `MEMBER_PRIV_KEY` if not set                                                         | False               | `0x1...`                                     |
| `TELEMETRY_PRIV_KEY_FILE`                                | Path to a file containing the private key for the telemetry account. Takes precedence over `TELEMETRY_PRIV_KEY`                                                             | False               | `/app/telemetry_private_key`                 |
...
| `FILEBASE_IPFS_TOKEN`                                    | Bearer token for Filebase IPFS authentication                                                                                                                               | False               | `eyJhbG...`                                  |
...
| `TX_GAS_ADDITION`                                        | Used to modify gas parameter that used in transaction. (gas = min(TX_GAS_LIMIT, estimated_gas + TX_GAS_ADDITION))                                                           | False               | `100000`                                     |
| `TX_GAS_LIMIT`                                           | Upper bound for the gas of a single transaction. The default is the EIP-7825 transaction gas cap (2^24).                                                                    | False               | `16777216`                                   |
...
| `EVENTS_SEARCH_STEP`                                     | Maximum length of a range for eth_getLogs method calls                                                                                                                      | False               | `7200`                                       |
...
| `PERFORMANCE_WEB_SERVER_MAX_EPOCH_RANGE`                 | Maximum epoch range accepted by the Performance Web API                                                                                                                     | False               | `6300`                                       |
...
| `PERFORMANCE_WEB_SERVER_LIMIT_MAX_REQUESTS`              | Restart a worker after N requests (unset = unlimited)                                                                                                                       | False               | `None`                                       |
| `PERFORMANCE_WEB_SERVER_LIMIT_CONCURRENCY`               | Max concurrent requests per worker; 503 over the limit (unset = unlimited)                                                                                                  | False               | `None`                                       |
...
| `VAULT_PAGINATION_LIMIT`                                 | The limit for getting staking vaults with pagination                                                                                                                        | False               | `100`                                        |
| `VAULT_VALIDATOR_STATUSES_BATCH_SIZE`                    | The limit for getting validators statuses in one request                                                                                                                    | False               | `100`                                        |
...
| `TELEMETRY_TX_SEND_TIMEOUT_SECONDS`                      | Total time budget to retry sending a telemetry transaction (refetching nonce and fee each attempt) before giving up                                                         | False               | `240`                                        |
...
Check out our [report divergence guide](docs/report-divergence.md) for comparing input fingerprints when members submit different report hashes.
